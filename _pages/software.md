...
`cbi_partitions` is a Python library implementing Conformalized Bayesian Inference (CBI) for clustering problems based on partition-valued MCMC output. It provides point estimation of data clustering, construction of a credible set of partitions with guaranteed posterior coverage using conformal prediction principles, and density-based clustering to explore posterior multimodality. The method is introduced in the paper ["Conformalized Bayesian Inference, with Applications to Random Partition Models"](https://arxiv.org/abs/2511.05746).
